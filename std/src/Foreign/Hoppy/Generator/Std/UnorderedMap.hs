...
module Foreign.Hoppy.Generator.Std.UnorderedMap (
  Options (..),
  defaultOptions,
  Contents (..),
  instantiate,
...
data Options = Options
  { optUnorderedMapClassFeatures :: [ClassFeature]
    -- ^ Additional features to add to the @std::unordered_map@ class.  UnorderedMaps are always
...
data Contents = Contents
  { c_map :: Class  -- ^ @std::unordered_map\<K, V>@
  , c_iterator :: Class  -- ^ @std::unordered_map\<K, V>::iterator@
  , c_constIterator :: Class  -- ^ @std::unordered_map\<K, V>::const_iterator@
  }
...
instantiate' mapName k v userReqs opts =
  let extName = toExtName mapName
      reqs = mconcat
             [ userReqs
             , reqInclude $ includeHelper "unordered_map.hpp"
             , reqInclude $ includeStd "unordered_map"
             ]
      ...
      map =
        (case (optKeyConversion opts, optValueConversion opts) of
           ...
           (maybeKeyConv, maybeValueConv) ->
             error $ concat
             ["Error instantiating std::unordered_map<", show k, ", ", show v, "> (external name ",
      ...
      iterator =
        addReqs reqs $
        makeForwardIterator Mutable Nothing $
        makeClass (identT' [("std", Nothing),
                            ("unordered_map", Just [k, v]),
                            ("iterator", Nothing)])
      ...
      constIterator =
        addReqs reqs $
        makeForwardIterator Constant Nothing $
        makeClass (identT' [("std", Nothing),
                            ("unordered_map", Just [k, v]),
          ...
          indent $ do
            ...
            indent $ do
              ...
              iterGetValue <- toHsClassEntityName iter $ case cst of
                Const -> "getValueConst"
                Nonconst -> "getValue"
              iterNext <- toHsClassEntityName iter "next"

              saysLn ["empty' <- ", mapEmpty, " this'"]
              sayLn "if empty' then HoppyP.return [] else do"
              indent $ do
                saysLn ["end' <- ", mapEnd, " this'"]
                saysLn ["iter' <- ", mapBegin, " this'"]
                sayLn "go' iter' end' []"
              sayLn "where"
              indent $ do
                  ...
                  indent $ do
                    ...
                    saysLn ["value' <- ",
                            case valueConv of
                              ...
                              ConvertValue -> "HoppyFHR.decode =<< ",
                            iterGetValue, " iter'"]
                    sayLn "go' iter' end' $ (key', value'):acc'"
