...
module Foreign.Hoppy.Documentation (
  -- * Overview
  -- $overview
...
Hoppy is a FFI generator for interfacing Haskell with C++.  It consists mainly
of a Haskell library, with interface definition data structures, code
generators, and some runtime support for Haskell bindings.  There is also a set
of supporting C++ include files.
...
for C++ things that she doesn't want to expose directly (in @\/lib@).
...
@--extra-lib-dirs=...\/lib@ to the @cabal configure@ for @\/hs@.
...
of existing C++ files (although some logical structure is recommended).  Modules
contain 'Export's which refer to concrete things that provide bindings.  Binding
definitions take advantage of Haskell's laziness, and can be highly circular,
the simplest case being a class that includes a method that makes use of the
class in its parameter or return types.
